from config.config import PRODUTO
...
def check_license(db, cliente):
    hostname, machine_guid = get_hostname_machine_guid()

    result = db.query("""
        SELECT data_vencimento, descricao_local FROM licencas
        WHERE hostname = ? AND machine_guid = ? AND status_licenca = 1 AND produto = ? AND cliente = ?
    """, [hostname, machine_guid, PRODUTO, cliente])
