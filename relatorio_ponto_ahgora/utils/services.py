...
class Services:
    ...
    def get_url_and_password(self, db, cliente, email) -> tuple[str, str]:
        
        result = db.query("""
            SELECT url, senha FROM usuarios WHERE cliente = ? AND usuario = ?
        """, [cliente, email])
        
        if result:
            return result[0]['url'], result[0]['senha']
